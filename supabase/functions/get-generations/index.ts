...
async function handler(req: Request) {
  // ⭐ Zod schema must be defined inside handler
  ...
  // --- SAME LOGIC BELOW (unchanged but now safe) ---
  ...
  const historyLimit =
    typeof profile?.limits?.history_limit === "number"
  ...
  if (payload.from) countQuery = countQuery.gte("created_at", payload.from);
  if (payload.to) countQuery = countQuery.lte("created_at", payload.to);
  ...
  const effectiveTotal =
    historyLimit !== null ? Math.min(totalRecords, historyLimit) : totalRecords;
  ...
  const rangeEnd =
    historyLimit !== null
      ? Math.min(offset + limit - 1, historyLimit - 1)
      : offset + limit - 1;
  ...
  if (payload.from) dataQuery = dataQuery.gte("created_at", payload.from);
  if (payload.to) dataQuery = dataQuery.lte("created_at", payload.to);
